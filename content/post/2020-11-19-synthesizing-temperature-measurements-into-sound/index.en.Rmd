...
This article shows how the [Chaos
...
live data is translated to sound.
...
used and how the terms roughly interact with each other.
...
With fluidsynth and a soundfont file midi notes can be synthesized to audio. The
soundfont specifies which sample is played for which midi note. An example how
to do that via python can be seen
...
and render it to an audio file that you were about to listen. 
...
The data in the csv files is produced during the loop. Only look into that if
you're interested in detail in the dictionary data structures that are used in
the main loop. This data can be visualized as follows:
