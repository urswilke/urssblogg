...
the hardware (a micro-controller and some thermometers connected to the PC), which is not very expensive. All that's audio-related is completely free.
...
widgetframe::frameWidget(graph_html)
```


#### Live plotter

Perhaps it's best to start with the data sent by the micro-controller. It's really easy to program a [live plotter in R](https://gitlab.com/urswilke/chaos_machine_code/-/blob/master/R/live_plotter/live_plotter.R) with the package [arduinor](https://github.com/r-arduino/arduinor). This yields an app shown here:
...
```{r echo=FALSE}
knitr::include_graphics("csv_vis-1.png")
```
...
```{r echo=FALSE}
knitr::include_graphics("midi_piano_roll-1.png")
```
